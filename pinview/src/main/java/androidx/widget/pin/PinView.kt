...
class PinView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = R.attr.pinViewStyle) : AppCompatEditText(context, attrs, defStyleAttr) {
    ...
    init {
        ...
        mPinItemHeight = a.getDimension(R.styleable.PinView_itemHeight,
                res.getDimensionPixelSize(R.dimen.pv_pin_view_item_size).toFloat()).toInt()
        mPinItemWidth = a.getDimension(R.styleable.PinView_itemWidth,
                res.getDimensionPixelSize(R.dimen.pv_pin_view_item_size).toFloat()).toInt()
    ...
    private fun drawPinView(canvas: Canvas) {
        ...
        for (i in 0 until mPinItemCount) {
            ...
            mPaint.color = if (highlight) getLineColorForState(*HIGHLIGHT_STATES) else currentLineColor
    ...
    private fun updatePinBoxPath(i: Int) {
        ...
        updateRoundRectPath(mItemBorderRect, mPinItemRadius.toFloat(), mPinItemRadius.toFloat(), drawLeftCorner, drawRightCorner)
    }
    ...
    private fun updateItemRectF(i: Int) {
        val halfLineWidth = mLineWidth.toFloat() / 2
        var left = scrollX.toFloat() + ViewCompat.getPaddingStart(this).toFloat() + (i * (mPinItemSpacing + mPinItemWidth)).toFloat() + halfLineWidth
